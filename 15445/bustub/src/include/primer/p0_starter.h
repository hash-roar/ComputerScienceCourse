 ...
 public:
  ...
  RowMatrix(int rows, int cols) : Matrix<T>(rows, cols) {
      ...
      for (int i = 0; i < rows; ++i) {
          data_[i] = this->linear_[rows*cols];
      }
  }
...
class RowMatrixOperations {
 public:
  ...
  static std::unique_ptr<RowMatrix<T>> Add(const RowMatrix<T> *matrixA, const RowMatrix<T> *matrixB) {
      ...
      auto result =  std::make_unique<RowMatrix<T>>();
      for (int i = 0; i < matrixA->GetRowCount(); ++i) {
          for (int j = 0; j < matrixA->GetColumnCount(); ++j) {
              result->SetElement(matrixA->GetElement(i,j)+matrixB->GetElement(i,j));
          }
      }
      return  result;
  }
  ...
  static std::unique_ptr<RowMatrix<T>> GEMM(const RowMatrix<T> *matrixA, const RowMatrix<T> *matrixB,
    ...
    // determine the dimension is right
  }
};
}  // namespace bustub
